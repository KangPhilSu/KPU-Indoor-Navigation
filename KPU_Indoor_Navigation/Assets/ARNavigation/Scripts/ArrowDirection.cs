...
public class ArrowDirection : MonoBehaviour
{
    ...
    public float rotationSmoothingSpeed;  //화살표가 회전하는 속도 설정
    ...
    void LateUpdate()    //어플리케이션의 모든 update메소드가 실행된 후에 LateUpdate메소드가 실행되기 때문에 사용자가 회전한 후에 그 값을 받아와 agent를 회전.
    {
        ...
        float rotationToApplyAroundY = targetEulerAngles.y + Agent.eulerAngles.y; //rotation값중 y값만 float형으로 저장
        //Debug.Log("SpherePointer : " + Agent.eulerAngles.y);
