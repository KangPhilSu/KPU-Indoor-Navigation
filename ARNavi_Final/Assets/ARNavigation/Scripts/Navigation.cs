...
public class Navigation : MonoBehaviour
{
    ...
    private LineRenderer line;           //맵 상에 경로를 표현하기 위한 LineRenderer타입 변수
    ...
    void Start()
    {
        ...
        if (start == false)
        {
            transform.position = GameObject.Find(startname).transform.position;
            ...
            transform.rotation = GameObject.Find(startname).transform.rotation;
        ...
        classList = GameObject.Find("ClassRoom").GetComponentsInChildren<ClassRoom>();
    ...
    void OnDrawGizmosSelected()    //경로 표시 메소드
    {
        ...
        var line = GetComponent<LineRenderer>();  //현재 인스턴스의 LineTenderer 컴포넌트를 var타입의 line변수에 정의
        if (line == null)
        {
    ...
    void Update()                                  //1프레임마다 실행되는 update메소드
    {
        ...
        for (int k = 0; k < classList.Length; k++)  //Roomlist배열의 길이만큼 for문 실행
        {
            ...
            {
                fin = classList[k].gameObject.name;
                break;
            }
